...
class EncoderGenerator(nn.Module):
    ...
    def forward(self, a):
        ...
        thetas = torch.cuda.FloatTensor(a_size).uniform_(0, np.pi)

        # Do we also add the real feature (i.e. a) to x ..?
        x = torch.empty(a_size[0], a_size[1], a_size[2], a_size[3], dtype=torch.cfloat, device=device)
        a = a.cpu()
        b = b.cpu()
        thetas = thetas.cpu()
        x = (a + b *1j) * torch.exp(1j * thetas)
